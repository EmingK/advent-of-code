...
impl Bingo {
    ...
    fn unmark(&mut self, val: i32) {
        self.board.iter_mut()
            .flat_map(|row| { row.iter_mut() })
            .filter(|grid| { grid.n == val })
            .for_each(|grid| { grid.marked = false; });
    }
...
fn main() {
    ...
    for n in input {
        ...
        if boards.iter().all(|board| { board.win() }) {
            boards.iter_mut().for_each(|board| {
                board.unmark(n);
            });
            let board = boards.iter_mut().find(|board| { !board.win() }).unwrap();
            board.mark(n);
            println!("{}", n * board.sum_unmarked());
            break;
        }
    }
}